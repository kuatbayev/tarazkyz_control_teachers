...
export function LandingPage({ onLogin }: { onLogin: () => void }) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!hasSupabaseConfig) {
      ...
      setError(
        'Supabase баптаулары табылмады. Localhost үшін .env.local ішіне VITE_SUPABASE_URL және VITE_SUPABASE_ANON_KEY қосыңыз, немесе уақытша VITE_ENABLE_LOCAL_AUTH_BYPASS=true орнатыңыз.',
      );
      setLoading(false);
      return;
    }
  ...
  return (
    <div className="relative flex min-h-screen flex-col overflow-hidden bg-[#0F172A] text-white">
      <div className="pointer-events-none absolute inset-0 opacity-10">
        <div className="absolute left-0 top-0 h-full w-full bg-[radial-gradient(#334155_1px,transparent_1px)] [background-size:40px_40px]"></div>
      </div>
      ...
      <main className="relative z-10 container mx-auto flex flex-1 flex-col items-center justify-center gap-16 px-6 py-12 lg:flex-row">
        <div className="max-w-2xl flex-1">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <h1 className="mb-6 bg-gradient-to-r from-white to-slate-400 bg-clip-text text-5xl font-extrabold leading-tight text-transparent lg:text-6xl">
              Мектептегі тәртіпті толық бақылауға арналған жүйе
            </h1>
            ...
            <div className="mb-10 grid grid-cols-1 gap-6 sm:grid-cols-2">
              {[
                ...
                <div key={i} className="flex items-center gap-3 text-slate-300">
                  <div className="rounded-lg border border-white/10 bg-white/5 p-2">{item.icon}</div>
          ...
          <div className="rounded-3xl border border-white/10 bg-white/5 p-8 shadow-2xl backdrop-blur-xl">
            ...
            <form className="space-y-6" onSubmit={handleLogin}>
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-white/10 bg-white/5 py-3 pl-11 pr-4 transition-all focus:outline-none focus:ring-2 focus:ring-blue-500"
              ...
              <div>
                ...
                <div className="relative">
                  <Settings className="absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-500" />
                  <input
                    type="password"
                    value={password}
              ...
              <button
                type="submit"
                disabled={loading}
                className="flex w-full items-center justify-center gap-2 rounded-xl bg-blue-600 py-4 font-bold text-white shadow-lg shadow-blue-600/20 transition-all active:scale-[0.98] hover:bg-blue-500 disabled:cursor-not-allowed disabled:opacity-50"
              >
                {loading ? (
                  <>
      ...
      <footer className="relative z-10 container mx-auto px-6 py-8 text-center text-sm text-slate-500">
