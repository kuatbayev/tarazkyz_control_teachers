...
export function DashboardSidebar({ activeTab, isSidebarOpen, onLogout, onSelectTab, profile }: SidebarProps) {
  const items = [
    ...
    <aside className={`flex flex-col bg-[#0F172A] text-white transition-all duration-300 ${isSidebarOpen ? 'w-64' : 'w-20'}`}>
      <div className="flex items-center gap-3 p-6">
        <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center overflow-hidden rounded-lg bg-white shadow-md">
                  <img src="/alt_logo-2.svg" alt="Taraz Girls BIL logo" className="h-full w-full object-cover" />
        </div>
        {isSidebarOpen && <span className="whitespace-nowrap text-lg font-bold tracking-tight">{profile.schoolName}</span>}
      </div>
      <nav className="flex-1 space-y-1 px-3 py-4">
        {items.map((item) => (
          <button key={item.id} onClick={() => onSelectTab(item.id)} className={`flex w-full items-center gap-4 rounded-xl px-4 py-3 transition-all ${activeTab === item.id ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`}>
      ...
      <div className="border-t border-white/5 p-4">
        <button onClick={onLogout} className="flex w-full items-center gap-4 rounded-xl px-4 py-3 text-slate-400 transition-all hover:bg-red-400/10 hover:text-red-400">
...
export function DashboardTopBar({ onToggleSidebar, profile, selectedTeacherName }: TopBarProps) {
  return (
    <header className="flex flex-shrink-0 items-center justify-between border-b border-slate-200 bg-white px-8 py-4">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-xl font-bold text-slate-800">{selectedTeacherName ? `Мұғалім профилі: ${selectedTeacherName}` : 'Мектеп шолуы'}</h1>
      </div>
      <div className="flex items-center gap-6">
        <div className="flex items-center gap-3 border-l border-slate-200 pl-6">
          ...
          <div className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-full border-2 border-blue-500 bg-blue-100">
            {profile.avatar ? <img src={profile.avatar} alt="Avatar" className="h-full w-full object-cover" /> : <Users className="h-5 w-5 text-blue-600" />}
          </div>
        </div>
      </div>
    </header>
